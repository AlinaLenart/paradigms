...
def indexSwap13[A](anyList: List[A]): List[A] = indexSwap(anyList, 1, 3)

val indexSwap24: [A] => List[A] => List[A] =
  [A] => (anyList: List[A]) => indexSwap(anyList, 2, 4)
...
indexSwap24(list1)
indexSwap24(list2)
indexSwap24(listEmpty)
indexSwap24(listOutOfIndex)